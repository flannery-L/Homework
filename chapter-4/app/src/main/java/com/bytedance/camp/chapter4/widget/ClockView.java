...
public class ClockView extends View {
    ...
    private void configWhenLayoutChanged() {
        ...
        for (int degree = 0; degree < FULL_CIRCLE_DEGREE; degree += d) {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        drawNeedle(canvas, 0, 5 * hour + minute / 12);
        drawNeedle(canvas, 1, minute);
        drawNeedle(canvas, 2, second);

    }
    private void drawNeedle(Canvas canvas, int NeedleType, int value) {
        float degree;
        float[] Header = new float[2];
        // 一个小格的度数
        float d = (float) (6 * Math.PI / 180);
        degree = value * d;
        switch (NeedleType) {
            case 0:
                needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
                Header = getHeader(radius * HOUR_NEEDLE_LENGTH_RATIO, degree);
                break;
            case 1:
                needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
                Header = getHeader(radius * MINUTE_NEEDLE_LENGTH_RATIO, degree);
                break;
            case 2:
                needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
                Header = getHeader(radius*SECOND_NEEDLE_LENGTH_RATIO, degree);
                break;
        }
        canvas.drawLine(centerX, centerY, Header[0], Header[1], needlePaint);
    }

    private float[] getHeader(float NeedleLength, float degree) {
        float[] header = new float[2];
        header[0] = (float) (centerX + NeedleLength * Math.sin(degree));
        header[1] = (float) (centerY - NeedleLength * Math.cos(degree));
        return header;
    }

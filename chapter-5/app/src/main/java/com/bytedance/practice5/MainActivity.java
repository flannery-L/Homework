...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                List<Message> Messages = baseGetMessagesFromRemote(
                        studentId);
    ...
    private List<Message> baseGetMessagesFromRemote(String studentId){
        String urlStr;
        if(studentId!=null){
            urlStr = Constants.BASE_URL+String.format("messages?student_id=%s", studentId);}
        else{
